...
export default async function YanEventsCityPage({ params }: { params: { city: string } }) {
  ...
  const events = await safeYanQuery(
    () =>
      prisma.yanEvent.findMany({
        where: { status: { in: ["coming-soon", "published", "past"] }, city: city.name },
                    ...
                    <Link href={`/yan/events/${city.slug}/${event.slug}`} className="yan-card block hover:shadow-md">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        {event.status === "coming-soon" && (
                          <span className="yan-pill !text-yan-clay !border-yan-clay/40 !bg-yan-clay/10 shrink-0">Coming soon</span>
                        )}
                      </div>
            ...
            {past.length > 0 && (
              <div className="mt-16">
                ...
                <ul className="space-y-3">
                  {past.map((event) => (
                    <li key={event.id}>
                      <Link href={`/yan/events/${city.slug}/${event.slug}`} className="text-yan-navy/60 hover:text-yan-blue text-sm">
  ...
  return (
    <>
      ...
      <section className="py-16 sm:py-20 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <YanStatsStrip stats={getCityStats("atlanta", ["loneliness", "engagement"])} />
        </div>
      </section>
